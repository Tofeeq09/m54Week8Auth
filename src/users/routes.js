...
userRouter.get("/authCheck", tokenCheck, login); // Define a GET route at the path "/authCheck". When this route is hit, the tokenCheck middleware function is called first, then the login controller function is called. The tokenCheck function checks if the token is valid, and the login function starts a new session if the token is valid.
...
userRouter.put("/:username", updateUserByUsername); // Define a PUT route at the path "/:username". When this route is hit, the updateUserByUsername controller function is called. This function updates the user with the specified username in the database and sends the updated user in the response.
userRouter.delete("/:username", deleteUserByUsername); // Define a DELETE route at the path "/:username". When this route is hit, the deleteUserByUsername controller function is called. This function deletes the user with the specified username from the database and sends a success message in the response.
// Sign up Route
